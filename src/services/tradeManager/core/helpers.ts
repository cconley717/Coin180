...
export function makeBufferReader(idxOf: (x: number, y: number) => number): BufferReader {
...
export function computeShadeCutoffs(greenL: number[], redL: number[], opts: HeatmapAnalyzerOptions) {
...
export function detectUniformShades(greenL: number[], redL: number[], opts: HeatmapAnalyzerOptions) {
...
export function pass2Classify(p: Pass2Params) {
  ...
  for (let y = 0; y < height; y += options.pixelStep) {
    for (let x = 0; x < width; x += options.pixelStep) {
      ...
      const shade =
        fam === FAMILY_GREEN
          ? (cuts.forceGreenShade ?? shadeFromL(L, cuts.gB1, cuts.gB2))
          : (cuts.forceRedShade ?? shadeFromL(L, cuts.rB1, cuts.rB2));
...
function familyOf(r: number, g: number, b: number, a: number, opts: HeatmapAnalyzerOptions): Family {
  if (a < 8) return FAMILY_NEUTRAL;
  else {
    const { h, s, v } = rgbToHsv(r, g, b);

    if (v < opts.minValue || s < opts.minSaturation) return FAMILY_NEUTRAL;
    else if (isGreenHue(h, opts.greenHueMin, opts.greenHueMax)) return FAMILY_GREEN;
    else if (isRedHue(h, opts.redHueLowMax)) return FAMILY_RED;
