...
import { FAMILY_GREEN, FAMILY_NEUTRAL, FAMILY_RED, 
    type HeatmapAnalyzerResult, type BufferReader, type CountMap, type Family, 
    type Pass2Params, type RGBA, type Shade, type ShadeTallies } from './types.js';
import type { HeatmapAnalyzerOptions } from './options.js';

export function makeBufferReader(
    idxOf: (x: number, y: number) => number
): BufferReader {
...
export async function loadRawImage(input: Buffer | string) {
...
export async function loadBlurredImage(input: Buffer | string, sigma: number): Promise<Buffer> {
    const blurred = await sharp(input).blur(sigma).raw().ensureAlpha().toBuffer({ resolveWithObject: true });

    return blurred.data;
}

export function autoTuneMinSaturationPass(
    src: Buffer,
    width: number,
    height: number,
    ...
    if (sSamples.length >= 50) {
        ...
        const sP = percentile(sSamples, Math.min(0.95, Math.max(0.05, opts.autoTuneSPercentile)));
...
export function pass1CollectLightness(
    src: Buffer,
    width: number,
    height: number,
    opts: HeatmapAnalyzerOptions,
    ...
    for (let y = 0; y < height; y += opts.pixelStep) {
        for (let x = 0; x < width; x += opts.pixelStep) {
            const [r, g, b, a] = getRGBAFn(src, x, y);
            const fam = familyOf(r, g, b, a, opts);
...
export function computeShadeCutoffs(
    greenL: number[],
    redL: number[],
    opts: HeatmapAnalyzerOptions
) {
    ...
    if (greenL.length >= 10) {
        gB1 = percentile(greenL, 0.33)!;
        gB2 = percentile(greenL, 0.66)!;
    }

    if (redL.length >= 10) {
        rB1 = percentile(redL, 0.33)!;
        rB2 = percentile(redL, 0.66)!;
    }

    const widenIfCollapsed = (b1: number, b2: number, arr: number[]) => {
        if (Math.abs(b2 - b1) < opts.collapseEps) {
            const med = percentile(arr, 0.5)!;
...
export function detectUniformShades(
    greenL: number[],
    redL: number[],
    opts: HeatmapAnalyzerOptions
) {
    ...
    const uniformShade = (arr: number[]) => {
        if (!opts.uniformDetect || arr.length < 50)
            return null;
        else {
            ...
            if (spread >= opts.uniformSpreadMax)
                return null;
            else {
                const med = percentile(arr, 0.5)!;

                if (med >= opts.uniformLightL)
                    return 'light';
                else if (med <= opts.uniformDarkL)
                    return 'dark';
                else
                    return 'medium';
            }
    ...
    if (gUniform)
        forceGreenShade = gUniform;

    const rUniform = uniformShade(redL);

    if (rUniform)
        forceRedShade = rUniform;

    return { forceGreenShade, forceRedShade };
}

export function pass2Classify(p: Pass2Params) {
    ...
    const hasNeighborAgreement = (x: number, y: number, fam: typeof FAMILY_GREEN | typeof FAMILY_RED): boolean => {
        ...
        for (let dy = -1; dy <= 1; dy++) {
            for (let dx = -1; dx <= 1; dx++) {
                ...
                const nf = familyOf(nr, ng, nb, na, options);
    ...
    const bump = (m: { green: ShadeTallies; red: ShadeTallies }, fam: typeof FAMILY_GREEN | typeof FAMILY_RED, shade: Shade) => {
        const side = m[fam];
    ...
    for (let y = 0; y < height; y += options.pixelStep) {
        for (let x = 0; x < width; x += options.pixelStep) {
            ...
            const fam = familyOf(r, g, b, a, options);

            if (fam === FAMILY_NEUTRAL)
                continue;

            if (!hasNeighborAgreement(x, y, fam))
                continue;

            const L = hslLightness(r, g, b);

            const shade = fam === FAMILY_GREEN ? (cuts.forceGreenShade ?? shadeFromL(L, cuts.gB1, cuts.gB2)) : (cuts.forceRedShade ?? shadeFromL(L, cuts.rB1, cuts.rB2));
...
export function mergeSmall(b: { light: number; medium: number; dark: number; total: number }, minShare: number) {
    if (b.total === 0)
        return;
...
export function computePercentages(counts: HeatmapAnalyzerResult['counts']) {
...
export function computeScore(counts: HeatmapAnalyzerResult['counts'], opts: HeatmapAnalyzerOptions) {
...
function hslLightness(r: number, g: number, b: number) {
    const rn = r / 255, gn = g / 255, bn = b / 255;
...
function isRedHue(h: number, redHueLowMax: number) {
    return (h >= 0 && h <= redHueLowMax) || (h >= 360 - redHueLowMax && h < 360);
}
function isGreenHue(h: number, min: number, max: number) {
    return h >= min && h <= max;
}

function shadeFromL(L: number, b1: number, b2: number): Shade {
    if (L >= b2)
        return 'light';
    else if (L >= b1)
        return 'medium';
...
function familyOf(
    ...
    if (a < 8)
        ...
        if (v < opts.minValue || s < opts.minSaturation)
            return FAMILY_NEUTRAL;
        else if (isGreenHue(h, opts.greenHueMin, opts.greenHueMax))
            return FAMILY_GREEN;
        else if (isRedHue(h, opts.redHueLowMax))
            return FAMILY_RED;
        else
            return FAMILY_NEUTRAL;
    }
