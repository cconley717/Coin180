...
async function parseLogFile(logFilePath: string): Promise<AnalyzerStats> {
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      // SlopeSignAnalyzer
      updateSignalCount(stats.slopeSignAnalyzer, data.tick.slopeSignAnalyzer.result.tradeSignal);

      // MomentumCompositeAnalyzer
      updateSignalCount(stats.momentumCompositeAnalyzer, data.tick.momentumCompositeAnalyzer.result.tradeSignal);

      // MovingAverageAnalyzer
      updateSignalCount(stats.movingAverageAnalyzer, data.tick.movingAverageAnalyzer.result.tradeSignal);

      // TradeSignalFusion
      updateSignalCount(stats.tradeSignalFusion, data.tick.tradeSignalFusion.result.tradeSignal);
    } catch {
...
function printStats(stats: AnalyzerStats): void {
  ...
  console.log(
    `  Total:    ${stats.tradeSignalFusion.buy + stats.tradeSignalFusion.sell + stats.tradeSignalFusion.neutral}\n`
  );
}
